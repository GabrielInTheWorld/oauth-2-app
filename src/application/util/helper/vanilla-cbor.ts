export default (function() {
  ...
  const enforceUint8Array = (buffer: any) => {
    ...
    if (bufferType == 'ArrayBuffer' || bufferType == 'Array') return new Uint8Array(buffer.slice());
    else return new Uint8Array(buffer.buffer.slice());

    // return temp;
  };

  const mergeTwoBuffers = function(buffer1: any, buffer2: any) {
  ...
  const getTLVForNext = (buffer: any) => {
    ...
    let tagnum: any = (buffer[0] - lennum) >> 5;
    let VAL = undefined;
    let TAG: any = tags[tagnum] as any;
  ...
  let removeTagValue = (num: any) => {
    return num - (num & 32) - (num & 64) - (num & 128);
  };

  let bufferToHex = (buffer: any) => {
  ...
  let findLegthOfIndefiniteLengthBuffer = (buffer: any) => {
    for (let i = 0; i < buffer.length; i++) {
      if (buffer[i] === 0xff) return i + 1;
    }
  };

  let decodeCborStream = (buffer: any, expectedLength: any): any => {
    buffer = buffer.slice();
